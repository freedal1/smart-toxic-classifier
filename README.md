...
|-------:|----------:|----------:|---------:|----------:|
| 1 | 0.908 | 0.865 | 0.861 | 0.863 |
| 2 | 0.912 | 0.888 | 0.845 | 0.866 |
| 3 | 0.917 | 0.890 | 0.860 | 0.875 |
| 4 | **0.920** | **0.870** | **0.893** | **0.881** |
...
Особенно важным является высокий показатель **Recall (89.3%)**, что означает, что модель успешно обнаруживает большую часть токсичных комментариев.
...
|---------|----------:|--------:|---------:|
| Non-toxic | **0.9456** | **0.9327** | **0.9391** |
| Toxic | **0.8698** | **0.8933** | **0.8814** |

### Вывод

Модель демонстрирует стабильное качество для обоих классов.

Для токсичных комментариев удалось получить:

- Precision — **86.98%**
- Recall — **89.33%**
- F1-score — **88.14%**

Высокое значение Recall особенно важно для задач автоматической модерации, поскольку позволяет обнаруживать большую часть нежелательных сообщений.
...
|-----------:|----------:|--------:|---------:|
| 0.1 | 0.860 | **0.907** | **0.883** |
| 0.2 | 0.865 | 0.901 | 0.882 |
| 0.3 | 0.868 | 0.898 | 0.883 |
| 0.4 | 0.869 | 0.894 | 0.882 |
| **0.5** | **0.870** | **0.893** | **0.881** |
| 0.6 | 0.873 | 0.891 | 0.882 |
| 0.7 | 0.874 | 0.885 | 0.880 |
| 0.8 | 0.879 | 0.881 | 0.880 |
| 0.9 | **0.884** | 0.867 | 0.876 |
...
Несмотря на то, что максимальное значение F1-score было достигнуто при пороге **0.1**, разница с порогом **0.5** минимальна (**0.8829 против 0.8814**).
...
- ⏳ WordCloud для токсичных комментариев
- ⏳ WordCloud для нетоксичных комментариев
- ⏳ Анализ наиболее частых токенов по каждому классу
- ⏳ Анализ средней длины комментариев для каждого класса
- ⏳ ROC Curve и ROC-AUC
- ⏳ K-Fold Cross Validation
- ⏳ Подбор оптимальных гиперпараметров (Learning Rate, Batch Size, Epochs)
- ⏳ Эксперименты с другими трансформерными моделями (ruBERT-base, XLM-RoBERTa)
- ⏳ SHAP-анализ интерпретируемости модели
- ⏳ Анализ ошибок по категориям токсичности
- ⏳ Исследование влияния размера `max_length` на качество модели
